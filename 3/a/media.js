...
const chargerMedia = (mediaType, mediaId) => {
  if (id.length === 0 || mediaType.length === 0) {
    alert("Il y a un souci dans l'URL !");
  } else {
    const tmdbUrl = `https://api.themoviedb.org/3/${mediaType}/${mediaId}?api_key=97719463bea4bd4b5902c1a735c0556a&language=fr-FR`;
    axios
      .get(tmdbUrl)
      .then((result) =>
        mediaType == "movie"
...
export const launchMedia = (mediaType, mediaId) => {
